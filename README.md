...
When a task needs the GUI — not just files and the shell — a role can take a screenshot and then click, type, scroll, and press keys to operate other macOS apps through the `screen_capture`, `ui_click`, `ui_type`, `ui_key`, and `ui_scroll` tools. Each screenshot comes back with an accessibility map of the on-screen controls and their exact click points — web content inside a browser is labeled separately from the browser's own chrome — so the model aims at real elements instead of guessing pixels, and text-only main models get the screenshot described by your **Vision model** automatically. Computer Use ships **manual by default**: every click, keystroke, and scroll pauses for your **Allow / Deny** right in the activity feed, with a preview of exactly what will run. Switch a work folder to **Auto** and an on-device LLM judge approves or denies each action at a **Safety** level — Permissive, Standard, Strict, or Off to skip the judge — or turn the feature fully **Off** to strip the tools from every role. A hard-deny layer applies in every mode: the tools can never target NanoTeams itself, clicks outside the captured screenshot are rejected, and an optional allowed-apps list plus blocked typed-text and key-combo patterns force-deny anything you list. On by default for the **Assistant**, **Coding Assistant**, and **Autovisor** roles; grant others per role in the Team Editor. macOS asks for Screen Recording and Accessibility permission the first time — configure everything in **Settings → Computer Use**.
...
### 50 Built-in Tools
Sandboxed tool system: file operations, shell command execution (`bash` / `bash_output`), git, Xcode build & test, team collaboration (`ask_teammate`, `request_team_meeting`, `request_changes`), team delegation (`delegate_to_team` with cancel/resume/forward follow-ups), team generation (`create_team`), task management for the Autovisor, artifact creation, supervisor Q&A, persistent memory, image analysis, and screen control (`screen_capture`, `ui_click`, `ui_type`, `ui_key`, `ui_scroll`).
...
### Privacy & Security
**NanoTeams** doesn't send your data anywhere. All processing happens locally via LM Studio. Debug logs are off by default. All file operations are sandboxed to the selected work folder. Shell access via the `bash` tool is **manual by default** — you approve every command before it runs — and is gated by a permission engine and an on-device LLM safety judge, then confined by a macOS Seatbelt sandbox that keeps writes inside your work folder and blocks credential reads. Screen control via the Computer Use tools is **manual by default** too — you approve every click, keystroke, and scroll with a preview, and can hand approval to an on-device safety judge or turn the feature off entirely; it uses only macOS Screen Recording and Accessibility permission and streams nothing off your Mac.
...
**Can NanoTeams control other apps?**
Yes — the Computer Use tools let a role take a screenshot and then click, type, scroll, and press keys to operate other macOS apps when a task needs the GUI. It's **manual by default** (you approve every action with a preview of what will run), can be handed to an on-device safety judge in Auto mode, or turned off entirely. Each screenshot includes an accessibility map so the model aims at real controls instead of guessing pixels. It uses only macOS Screen Recording and Accessibility permission, nothing leaves your Mac, and it's on by default for the Assistant, Coding Assistant, and Autovisor roles.
