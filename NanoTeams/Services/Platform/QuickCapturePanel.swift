...
class QuickCapturePanel: NSPanel, NSWindowDelegate {
    ...
    #if DEBUG
    /// How many form views have been installed.
    ///
    /// `contentView` cannot answer this from a test: the panel installs one in its own
    /// init, and the type `setContent` produces is a generic `NSHostingView<some View>`
    /// whose parameter is unspellable outside this method. Without the counter, "the
    /// overlay rendered blank" and "the overlay rendered the form" are indistinguishable
    /// — which is exactly the failure `updatePanelContent`'s three-way guard produces.
    private(set) var _testSetContentCount = 0
    var _testHasContent: Bool { _testSetContentCount > 0 }
    #endif

    /// Sets the SwiftUI content view for this panel.
    func setContent<Content: View>(_ view: Content) {
        #if DEBUG
        _testSetContentCount += 1
        #endif
    ...
    func show(expectsFocusableField: Bool) {
        ...
        //
        // Measured 2026-08-10, so nobody re-spends the afternoon: on macOS 26 this block is a
        // REDUNDANT second net, which is why its three lines never execute under test. Seed a
        // sub-floor `NSWindow Frame <name>` record, construct the panel, `show` it — the frame
        // comes back at the floor with this block deleted, with `clampSize` neutered to the
        // identity, and with the `setFrameUsingName` call removed. `minSize`, set in `init`, is
        // what enforces it, and NSWindow applies it across every restore path a test can reach.
        // A test written against these lines therefore pins AppKit rather than us and passes on
        // every mutation; the block stays because the floor is a user-visible promise and a
        // second net costs nothing, not because anything here is known to need it.
        var clamped = frame
    ...
    /// - Parameter pullKeyBack: whether to take key focus back before restoring the
    ///   responder. Two separable concerns that used to be one: restoring the responder is
    ///   ALWAYS right after a content swap, but taking key is right only when the user
    ///   asked for the panel. A passive rebuild (engine transition, post-submit mode flip)
    ///   must not yank focus out of the main window — and it does not need to: setting the
    ///   first responder on a non-key window is legal, so the caret is simply waiting when
    ///   the panel next becomes key.
    func refocusInputField(pullKeyBack: Bool = true) {
        #if DEBUG
    ...
    /// Counts the refocus calls that ALSO took key focus back. Separate from the
    /// invocation count because the two halves of `refocusInputField` are now separable
    /// and only one of them is safe on a passive rebuild — a single counter cannot tell
    /// "restored the caret" from "stole the keyboard".
    private(set) var _testKeyPullCount = 0
