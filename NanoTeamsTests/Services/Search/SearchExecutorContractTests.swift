...
final class SearchExecutorContractTests: XCTestCase {
    ...
    private func list(maxResults: Int = 20, offset: Int = 0) throws -> SearchExecutorOutput {
        try run([""], maxResults: maxResults, offset: offset)
    }
    ...
    func testSymlinkCycle_terminates() throws {
    ...
    func testSymlinkCycle_terminatesEvenWithNoMatchToStopOn() throws {
        try write("a/real.swift", content: "unrelated\n")
        try fm.createSymbolicLink(
            at: tempDir.appendingPathComponent("a/loop"),
            withDestinationURL: tempDir.appendingPathComponent("a"))

        let out = try run(["zzz_absent"])
    ...
    func testSymlinkCycle_isReportedNotSwallowed() throws {
        try write("a/real.swift", content: "NEEDLE\n")
        try fm.createSymbolicLink(
            at: tempDir.appendingPathComponent("a/loop"),
            withDestinationURL: tempDir.appendingPathComponent("a"))
    ...
    func testSymlinkedDirectory_isStillSearched() throws {
    ...
    func testSymlinkOutsideTheWorkFolder_isNotFollowed() throws {
    ...
    func testSymlinkToFileOutsideTheWorkFolder_isNotRead() throws {
        let outside = fm.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try fm.createDirectory(at: outside, withIntermediateDirectories: true)
        defer { try? fm.removeItem(at: outside) }
    ...
    func testSymlinkIntoTheInternalDirectory_isNotFollowed() throws {
        try "NEEDLE secret\n".write(
    ...
    func testSymlinkToAFileInsideTheInternalDirectory_isNotRead() throws {
        try "NEEDLE secret\n".write(
    ...
    func testRelativeSymlink_insideTheFolder_isFollowed() throws {
    ...
    func testRelativeSymlink_escapingTheFolder_isRefused() throws {
        let outside = fm.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try fm.createDirectory(at: outside, withIntermediateDirectories: true)
        defer { try? fm.removeItem(at: outside) }
    ...
    func testSymlinkToASiblingWhoseNamePrefixesTheRoot_isRefused() throws {
    ...
    func testSymlinkToFileInsideTheFolder_isSearched() throws {
    ...
    func testSymlinkInsideASymlinkedDirectory_isHandled() throws {
    ...
    func testListMode_symlinkedDirectory_appearsOnce() throws {
        try write("real/a.swift", content: "x\n")
        try fm.createSymbolicLink(
            at: tempDir.appendingPathComponent("alias"),
            withDestinationURL: tempDir.appendingPathComponent("real"))

        let out = try list(maxResults: 50)
    ...
    func testDanglingSymlink_isSkippedQuietly() throws {
    ...
    func testWalk_distinctDirectoriesWithTheSameName_areBothWalked() throws {
    ...
    func testWalk_symlinkToSiblingSubtree_isNotDoubleCounted() throws {
    ...
    func testListMode_saturated_reportsNoTotal() throws {
        for i in 0..<30 { try write("f\(i).swift", content: "x\n") }

        let out = try list(maxResults: 5)
    ...
    func testTotalAndHasMore_areNeverBothPresent() throws {
        for i in 0..<12 { try write("f\(i).swift", content: "NEEDLE\n") }

        for size in [1, 5, 11, 12, 13, 50] {
            for offset in [0, 5, 20] {
                for out in [try run(["NEEDLE"], maxResults: size, offset: offset),
    ...
    func testListMode_complete_reportsTheRosterTotal() throws {
    ...
    func testListMode_pageCountCountsNames_notContentMatches() throws {
        for i in 0..<30 { try write("f\(i).swift", content: "x\n") }
    ...
    func testWheneverHasMoreIsSet_pageCountIsNonZero() throws {
    ...
    func testListMode_pagingByCount_partitionsTheRoster() throws {
        for i in 0..<17 { try write(String(format: "f%02d.swift", i), content: "x\n") }

        var seen: [String] = []
        var offset = 0
        for _ in 0..<20 {
            let out = try list(maxResults: 5, offset: offset)
    ...
    func testContentMode_filenameMatches_doNotRepeatOnLaterPages() throws {
        ...
        let first = try run(["NEEDLE"], maxResults: 2, offset: 0)
        let second = try run(["NEEDLE"], maxResults: 2, offset: 2)
    ...
    func testContentMode_filenameMatchesCut_emitsAWarning() throws {
        for i in 0..<12 { try write("NEEDLE_\(i).swift", content: "unrelated\n") }

        let out = try run(["NEEDLE"], maxResults: 3)
    ...
    func testContentMode_filenameMatchesComplete_emitsNoWarning() throws {
        for i in 0..<3 { try write("NEEDLE_\(i).swift", content: "unrelated\n") }

        let out = try run(["NEEDLE"], maxResults: 50)
        XCTAssertEqual(out.filenameMatches.count, 3)
    ...
    func testListMode_offsetBeyondEnd_isAnEmptyPageNotAnError() throws {
        for i in 0..<3 { try write("f\(i).swift", content: "x\n") }

        let out = try list(maxResults: 5, offset: 500)
    ...
    func testPageSizeOne_pagesOneAtATime() throws {
        for i in 0..<3 { try write("f\(i).swift", content: "NEEDLE\n") }

        var seen: [String] = []
        var offset = 0
        for _ in 0..<10 {
            let out = try run(["NEEDLE"], maxResults: 1, offset: offset)
    ...
    func testPageExactlyFull_doesNotPromiseAnotherPage() throws {
    ...
    func testOrdinarySearch_carriesNoWarnings() throws {
    ...
    func testContentModeLaterPage_carriesNoFilenameWarning() throws {
        for i in 0..<12 { try write("NEEDLE_\(i).swift", content: "NEEDLE\n") }

        let out = try run(["NEEDLE"], maxResults: 3, offset: 3)
        XCTAssertTrue(out.filenameMatches.isEmpty)
    ...
    func testNoMatches_reportsAnExactTotalOfZero() throws {
    ...
    func testEmptyWorkFolder_isAComplete_emptyResult() throws {
        let out = try run(["NEEDLE"])
        XCTAssertTrue(out.matches.isEmpty)
        XCTAssertFalse(out.truncated)
        XCTAssertEqual(out.totalMatches, 0)
    ...
    func testPerQueryCapHit_withholdsTheExactTotal() throws {
    ...
    func testSingleQuery_completeSearch_stillReportsTheExactTotal() throws {
        try write("some.swift", content: String(repeating: "ALPHA\n", count: 6))

        let out = try run(["ALPHA"], maxResults: 30)
        XCTAssertFalse(out.truncated)
