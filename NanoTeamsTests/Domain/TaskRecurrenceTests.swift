...
final class TaskRecurrenceTests: XCTestCase {
    ...
    func testReschedule_pastOnceDisablesAndClearsNextFire() {
        var rec = TaskRecurrence(rule: .once(date: date(2026, 1, 1, 7, 0)))
        rec.reschedule(after: date(2026, 1, 1, 8, 0), calendar: cal)
    ...
    func testReschedule_futureOnceStaysEnabled() {
        var rec = TaskRecurrence(rule: .once(date: date(2026, 1, 1, 9, 0)))
        rec.reschedule(after: date(2026, 1, 1, 8, 0), calendar: cal)
        XCTAssertEqual(rec.nextFireAt, date(2026, 1, 1, 9, 0))
        XCTAssertTrue(rec.isEnabled)
    }
    ...
    func testCodableRoundTrip() throws {
        let rec = TaskRecurrence(
            rule: .dailyAt(hour: 9, minute: 30, weekdays: [2, 4, 6]),
            isEnabled: true,
            nextFireAt: date(2026, 1, 2, 9, 30),
            lastFiredAt: date(2026, 1, 1, 9, 30)
        )
    ...
    func testReschedule_dailyAt() {
        var rec = TaskRecurrence(rule: .dailyAt(hour: 9, minute: 0, weekdays: []))
        rec.reschedule(after: date(2026, 1, 1, 8, 0), calendar: cal)
        XCTAssertEqual(rec.nextFireAt, date(2026, 1, 1, 9, 0))
        XCTAssertTrue(rec.isEnabled)
    }
    ...
    func testReschedule_monthly() {
        var rec = TaskRecurrence(rule: .monthlyAt(day: 20, hour: 9, minute: 0))
        rec.reschedule(after: date(2026, 1, 15, 10, 0), calendar: cal)
        XCTAssertEqual(rec.nextFireAt, date(2026, 1, 20, 9, 0))
    }
    ...
    func testReschedule_degenerateRepeatingRule_selfDisables() {
        ...
        var rec = TaskRecurrence(rule: .dailyAt(hour: 9, minute: 0, weekdays: [8]))
        rec.reschedule(after: date(2026, 1, 1, 8, 0), calendar: cal)
    ...
    func testReschedule_healthyRepeatingRule_staysEnabled() {
        ...
        var rec = TaskRecurrence(rule: .dailyAt(hour: 9, minute: 0, weekdays: [6]))
        rec.reschedule(after: date(2026, 1, 1, 8, 0), calendar: cal)
